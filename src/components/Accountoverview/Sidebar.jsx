...
const footerItems =
  activeItem === "Events Manager"
    ? [
        { icon: <Settings size={40} />, label: "Business Setting", action: () => {}, tooltip: "Business Setting" },
        { icon: <Bell size={40} />, label: "Notifications", action: () => setShowNotifications(true), tooltip: "Notifications" },
        { icon: <Search size={40} />, label: "Search", action: () => setShowSearch(true), tooltip: "Search" },
        { icon: <HelpCircle size={40} />, label: "Help", action: () => setShowHelp(true), tooltip: "Help" },
        { icon: <Bug size={40} />, label: "Report Bug", action: () => setShowReportBug(true), tooltip: "Report Bug" },
      ]
    : [
        { icon: <BookOpen size={40} />, label: "Add Account Update", action: () => setShowAddAccount(true), tooltip: "Add Account Update" },
        { icon: <Settings size={40} />, label: "Business Setting", action: () => {}, tooltip: "Business Setting" },
        { icon: <Search size={40} />, label: "Search", action: () => setShowSearch(true), tooltip: "Search" },
        { icon: <Bug size={40} />, label: "Report Bug", action: () => setShowReportBug(true), tooltip: "Report Bug" },
      ]
...
>
  ...
  <div className="flex items-center gap-3">
    {!sidebarExpanded ? (
      ...
      <img src="/meta.png" alt="∞" className="w-9 h-5 w-fit mb-5" />
    ) : (
      // Expanded: Smaller logo, Meta text, Ads Manager below
      <div className="flex flex-col">
        <div className="flex items-center gap-2">
          <img src="/meta.png" alt="∞" className="w-6 h-3" />
          <span className="font-semibold text-black text-base">Meta</span>
        </div>
        <span className="text-xl font-bold text-black mt-1">Ads Manager</span>
      </div>
    )}
  </div>
</div>
...
{sidebarExpanded && (
  activeItem === "Events Manager" && (
    ...
    <div className="px-4 py-3 border-b border-gray-200">
      <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium text-gray-900">Adkin Digital LTD</div>
          <div className="text-xs text-gray-500">ADKI DIGI Digital Media</div>
        </div>
      ...
      icon={
        <img
          src={activeItem === item.id ? item.icon.light : item.icon.dark}
          alt={item.label}
          width={32}
          height={32}
        />
    ...
    active={showAllTools}
    onClick={() => handleItemClick(sidebarItems[sidebarItems.length - 1])} // ✅ pass full object
    sidebarExpanded={sidebarExpanded}
  />
        </div>
</div>
...
/>
          ...
          <div className={`flex my-2  ${sidebarExpanded ? "flex-row px-6 gap-4" : "flex-col items-center gap-2"}`}>
...
/>   {/* All Tools Menu */}
      ...
      <AddAccountUpdateSidebar isOpen={showAddAccount} onClose={() => setShowAddAccount(false)} />
     
      <SearchDialog isOpen={showSearch} onClose={() => setShowSearch(false)} />
      <ReportBugDialog isOpen={showReportBug} onClose={() => setShowReportBug(false)} /></>
  )
} 